...
class Cx extends TagLib {
    ...
    /**
     * import 标签解析 <import file="Js.Public" />
     * <import file="Css.Public" type="css" />
     * @access public
     * @param array $tag 标签属性
     * @param string $content  标签内容
    ...
    // import别名 采用文件方式加载(要使用命名空间必须用import) 例如 <load file="__PUBLIC__/Js/Public.js" />
    public function _load($tag,$content) {
        return $this->_import($tag,$content,true);
    }

    // import别名使用 导入css文件 <css file="__PUBLIC__/Css/Public.css" />
    public function _css($tag,$content) {
        return $this->_import($tag,$content,true,'css');
    }

    // import别名使用 导入js文件 <js file="__PUBLIC__/Js/Public.js" />
